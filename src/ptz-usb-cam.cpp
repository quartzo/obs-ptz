...
public:
    DirectShowControl(const std::string& device) {
        ...
        hr = cam_control_->GetRange(CameraControl_Pan, &min.pan, &max.pan,
                                            &step, &default_value, &flags);
        if (!FAILED(hr)) {
            hr = cam_control_->GetRange(CameraControl_Tilt, &min.tilt, &max.tilt,
                                        &step, &default_value, &flags);
        }
        if (!FAILED(hr)) {
            hr = cam_control_->GetRange(CameraControl_Zoom, &min.zoom, &max.zoom,
                                        &step, &default_value, &flags);
        }
        if (!FAILED(hr)) {
            hr = cam_control_->GetRange(CameraControl_Focus, &min.focus, &max.focus,
                                        &step, &default_value, &flags);
        }
        if (FAILED(hr)) {
...
void PTZUSBCam::memory_recall(int i)
{
    ...
    if (!now_pos.focusAuto) focus_abs(now_pos.focus);
}
